...
// Setup database without any complex checks that cause crashes
async function initDb() {
    try {
        ...
        console.log("DB already exists or connection slow, moving on...");
    }
}
initDb();
...
// Individual Item Page (Polytoria.trade style)
app.get('/store/:id', async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) return res.status(404).send("Item not found. Sync data at /internal/update first.");
        const item = result.rows[0];

        res.send(`
            <html>
            <head><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
            <body style="font-family:sans-serif; background:#f0f2f5; display:flex; justify-content:center; align-items:center; min-height:100vh; margin:0; padding:20px;">
                <div style="background:white; max-width:800px; width:100%; border-radius:15px; padding:30px; display:flex; flex-wrap:wrap; gap:30px; box-shadow:0 4px 20px rgba(0,0,0,0.1);">
                    ...
                    <div style="flex:1.5; min-width:250px;">
                        ...
                        <a href="https://polytoria.com/store/${item.item_id}" style="display:block; text-align:center; background:#007bff; color:white; text-decoration:none; padding:15px; border-radius:8px; font-weight:bold;">Buy on Polytoria</a>
                        <br><a href="/" style="color:#666; text-decoration:none;">← Back to Catalog</a>
                    </div>
                </div>
            </body>
            </html>
